        ...
        if (postIds != null) {
            for (String postId : postIds) {
                ...
                Post post = postRepository.findById(postIdLong)
                        .orElseThrow(() -> new RuntimeException("Post not found. postId: " + postIdLong));
        ...
        for (Long checkUserId : myUserIds) {
            if (!redisUserIds.contains(checkUserId)) {
                User user = userRepository.findByUserId(checkUserId)
                        .orElseThrow(() -> new IllegalArgumentException("User not found. userId: " + checkUserId));
                Like like = likeRepository.findByUserAndPost(user, postRepository.findById(postIdLong)
                                .orElseThrow(() -> new RuntimeException("Post not found. postId: " + postIdLong)))
                        .orElseThrow(() -> new RuntimeException("Like not found for userId: " + checkUserId));
                ...
                LikeCount likeCount = likeCountRepository.findByPostId(postIdLong)
                        .orElseThrow(() -> new RuntimeException("LikeCount not found for postId: " + postIdLong));
                ...
                LikeCount likeCount = likeCountRepository.findByPostId(postIdLong)
                        .orElseGet(() -> {
                            LikeCount newLikeCount = LikeCount.builder()
                                    ...
                                    .count(0)
                                    .build();
                            likeCountRepository.save(newLikeCount); // 새로 생성된 객체 저장
                            return newLikeCount;
                        });
                likeCount.setCount(likeCount.getCount() + 1);
                likeCountRepository.save(likeCount);
